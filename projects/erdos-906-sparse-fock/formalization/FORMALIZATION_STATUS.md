...
Synchronized paper/formalization snapshot: September 17, 2026.

## Paper scope

The paper `../paper/paper_en.tex` fixes the explicit parameter value \(p=3/2\).

Every theorem, proposition, corollary, and the origin-multiplicity remark in the paper has a corresponding Lean declaration in `lean/RequestProject/`. Broader results are kept in `../research/` and are outside the formalized paper.
...
The repository CI performs the static integrity scan and builds `RequestProject.Main`. A passing run on a specified commit verifies the source state described by that commit.

The Lean toolchain is pinned to `leanprover/lean4:v4.28.0`; the Mathlib revision is pinned by the project manifest.

## Material outside the formalized paper

The following topics are not represented as machine-checked results in this paper:
