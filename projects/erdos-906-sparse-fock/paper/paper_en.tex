...
Let $H_j=T_j+T_{j+1}$.  At the model center $z_{j,\ell}$, the two summands cancel.  More precisely, for $z=z_{j,\ell}(1+u)$,
\begin{equation}\label{eq:model}
 \frac{H_j(z)}{T_j(z)}=1-(1+u)^{q_j}.
\end{equation}
Choose a sufficiently small fixed $c>0$.  On $|u|=c/q_j$, the right-hand side of \eqref{eq:model} is bounded below by a positive constant depending only on $c$, whereas \eqref{eq:tail} tends to zero exponentially in $n^{1/6}$.  Rouch\'e's theorem therefore gives exactly one zero, counted with multiplicity, in each disk \eqref{eq:modeldisk}.  In particular the zero is simple.  This proves Proposition~\ref{prop:modeldisk}.

The Lean proof of Proposition~\ref{prop:modeldisk} uses the same two-term and tail estimates but proves the local zero count and simplicity directly, rather than treating Rouch\'e's theorem as a library black box.  Thus the machine-checked statement is the same as the printed proposition.
